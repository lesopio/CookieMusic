...
## 2026-08-13 - 当前源码用户功能完整审查

### 本次完成的工作

- 以实际代码为准，审查 `app/src/main/java` 下全部 Kotlin 源码，并追踪 `MainActivity`、各页面、组件、`PlayerViewModel`、播放服务、数据仓库和数据库之间的调用链。
- 检查 `AndroidManifest.xml`、应用级 Gradle 配置、系统栏版本资源和单元测试所体现的行为。
- 区分已完整接入 UI/播放流程的功能、隐藏功能、实验性或未接入功能，以及纯内部技术实现。
- 确认当前仓库仅实现 Android 应用，没有 Windows 或鸿蒙客户端源码。
...
- `rg --files app/src/main app/src/test`
- `rg -n` 检索页面入口、交互控件、播放控制、歌词、音效、悬浮窗、数据库和实验性字段
- `Get-Content` 分段审阅 Manifest、Gradle、Kotlin 源码、资源和单元测试

### 当前存在的问题

- `AudioVisualizationState` 当前没有实际音频采样或更新链路，所谓低频脉冲不能作为真实随节拍联动功能宣传。
- 粒子、封面运动、进度粒子等若干动画设置字段未接入实际渲染，部分还会在加载/规范化时被强制关闭。
- `HighRefreshRateEffect` 已定义但未调用，不能算高刷新率功能。
- 部分设备上的系统 AudioEffect、悬浮窗、控制中心动态歌词和复杂歌词格式兼容性仍需要真机人工验证。

### 下一步建议

- 根据本次功能边界撰写项目介绍时，只把已接入 UI 与业务链的能力列为正式功能。
- 若要宣传节拍联动、粒子或封面运动，需先补齐真实音频可视化采样和对应渲染入口。
- 增加歌词解析、播放模式、歌单持久化、历史记录、导入授权和服务生命周期方面的自动化测试。
## 2026-08-13 - README 按 0.9.0 实际源码重写

### 本次完成的工作

- 根据完整源码审查结果重写项目 README，修复原有中文乱码。
- 更新项目定位、已实现功能、歌词格式、导入方式、播放与队列、收藏歌单、音效、主题、悬浮歌词和自适应布局说明。
- 新增权限说明、技术架构、源码构建命令、测试状态和当前限制。
- 删除或纠正源码中尚未生效的宣传，包括自动最高刷新率、真实节拍联动、粒子效果和未接入的播放手势。
- 明确当前仓库仅包含 Android 客户端，HiFi 模式不代表独立高解析解码或硬件认证。
...
- `Get-Content -Raw version.properties`
- `git diff --check`

### 当前存在的问题

- README 沿用原有 GitHub 附件截图，截图是否完全对应 0.9.0 最终界面需要人工确认。
- GitHub Releases 中是否已经存在可公开下载的 0.9.0 APK，需要发布前在 GitHub 页面确认。

### 下一步建议

- 为 0.9.0 重新截取首页、播放页、双语歌词、导入管理、均衡器和悬浮歌词截图。
- 发布 0.9.0 Release 时同步版本说明、APK 校验值和已知限制。
